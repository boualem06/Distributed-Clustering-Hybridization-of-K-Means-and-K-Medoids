        ...
        try {
            ...
            while (centroidsChanged && iterations < maxIterations) {
                ...
                for (int i = 0; i < numClients; i++) {
                    ...
                    if (i==(numClients-1)){
                        double[][] chunk = Arrays.copyOfRange(points, startIndex, points.length);
                ...
                //     System.out.println(totalResults[i][0]+"  "+totalResults[i][1]);
                // }
            ...
            double[][] medoids= new double[centroids.length][]; 
            ...
            for (int i = 0; i < centroids.length; i++) {
                ...
                medoids[i]=points[nearestIndex] ;
                System.out.println("Centroid " + (i + 1) + " is closest to point (" + points[nearestIndex][0] + ", " + points[nearestIndex][1] + ")");
                System.out.println("Centroid " + (i + 1) + " is closest to point (" + medoids[i][0] + ", " + medoids[i][1] + ")");
            ...
            for (int iteration = 0; iteration < maxIterations && !converged; iteration++) {
                ...
                for (int i = 0; i < numClients; i++) {
                    ...
                    if (i==(numClients-1)){
                        ...
                        // System.out.println("start index "+startIndex+"   end index"+ points.length);
                ...
                System.out.println("************** Results from Clients after calculating the distance between points and medoides: ****************");
                for (int i = 0; i < numClients; i++) {
                ...
                clientResultsCost = new double[numClients][][];

                 // ********************************Send each chunk to a client to calculate the cost for each point in cluster when it is medoid  ***************************
                for (int i = 0; i < numClients; i++) {
                    ClientInter client = (ClientInter) registry.lookup("client" + (i + 1));
                    int startIndex = i * chunkSize;
                    int endIndex = Math.min(startIndex + chunkSize, points.length);
                ...
                if(test==medoids.length){
                     System.out.println("the algorithm converged after  "+(iteration + 1) +" iterations");
                    converged=true ;
                }
